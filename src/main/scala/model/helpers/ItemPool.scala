...
class ItemPoolImpl extends ItemPool {
  ...
  override def getItem(entityBody: EntityBody, size: (Float, Float), poolName: ItemPools): ItemImpl = {
    ...
    pickedItem match {
      case Items.Cake => new CakeItem(EntityId.Immobile, entityBody, size)
      case Items.Wrench => new WrenchItem(EntityId.Immobile, entityBody, size)
      case Items.Map => new MapItem(EntityId.Immobile, entityBody, size)
      case Items.PotionS => new SmallPotionItem(EntityId.Immobile, entityBody, size)
      case Items.PotionM => new PotionItem(EntityId.Immobile, entityBody, size)
      case Items.PotionL => new LargePotionItem(EntityId.Immobile, entityBody, size)
      case Items.PotionXL => new HugePotionItem(EntityId.Immobile, entityBody, size)
      case Items.Armor => new ArmorItem(EntityId.ArmorItem, entityBody, size)
      case Items.Boots => new BootsItem(EntityId.Immobile, entityBody, size)
      case Items.BFSword => new BFSwordItem(EntityId.Immobile, entityBody, size)
      case Items.Key => new KeyItem(EntityId.Immobile, entityBody, size)
      case Items.SkeletonKey => new SkeletonKeyItem(EntityId.Immobile, entityBody, size)
      case Items.Bow => new BowItem(EntityId.Immobile, entityBody, size)
      case Items.Shield => new ShieldItem(EntityId.Immobile, entityBody, size)
      case _ => new CakeItem(EntityId.Immobile, entityBody, size)
    }
  }
