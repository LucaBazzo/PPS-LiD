...
class MainMenuScreen(private val observerManager: ObserverManager) extends Screen {
  ...
  mainTable.defaults().pad(DISTANCE_BUTTONS_DEFAULT)

  // Set background
  val backgroundTexture: Texture  = GUIFactory.createTexture("assets/backgrounds/background_main_menu.png")
  ...
  mainTable.add(GUIFactory.createLabel("", bitmapFont, Color.WHITE)).size(DEFAULT_DISTANCE_FROM_TOP)
  mainTable.row()
  ...
  buttons = this.addButtonListener(GUIFactory.createTextButton("Exit", exitStyle)) :: buttons

  val playStyle: TextButtonStyle =
    GUIFactory.createTextButtonStyle("play_button_inactive", "play_button_active", skin, bitmapFont)

  buttons = this.addButtonListener(GUIFactory.createTextButton("Play", playStyle)) :: buttons
